"""Family D rule recall on Tier 1 fixtures."""
...
def family_d_rules():
    from detector.rules.family_d_owner import RULES

    return RULES


@pytest.mark.parametrize("rule_id", FAMILY_D)
def test_family_d_mal_fires(slither_for, family_d_rules, rule_id: str) -> None:
    label = load_tier1_label(rule_id, "mal")
    ctx = tier1_ctx(slither_for, rule_id, "mal")
    findings = run_and_finalize(ctx, family_d_rules)
...
@pytest.mark.parametrize("rule_id", FAMILY_D)
def test_family_d_ben_after_finalize(slither_for, family_d_rules, rule_id: str) -> None:
...
@pytest.mark.parametrize("rule_id", FAMILY_D)
def test_family_d_file_verdict(rule_id: str) -> None:
