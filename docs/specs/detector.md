...
| ID | Criterion | Status |
|---|---|---|
| DT-1 | `detector` is a registered bench tool with a Docker image; `bench run detector` completes under `--network none` on all tiers; output is schema-valid; determinism passes | met 2026-09-20 — Docker all-tier `--network none --repeat 2` (image from `6aee5fb`): 0.9714, schema-valid, determinism pass (`reports/detector/`) |
