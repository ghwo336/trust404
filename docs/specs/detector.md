...
- `INSTALLED_SOLC` is loaded at import from `detector/solc_versions.txt` (SSOT; the Dockerfile and `scripts/setup_local.sh` read the same file): `0.4.24`–`0.4.26`, `0.5.16`–`0.5.17`, `0.6.6`, `0.6.12`, `0.7.6`, and contiguous `0.8.0`–`0.8.37`. `DEFAULT_SOLC = "0.8.20"`. Widening the set required the `pick_solc` rule change in the next bullet (Wave B4, 2026-09-20): range pragmas keep their anchor picks (`>=0.4.22 <0.6.0` → `0.4.26`), only exact / tight pragmas move to a newly native pin.
- `pick_solc(source: str) -> str` — constraint-aware. Every `pragma solidity` statement in the file is parsed (`^`, `~`, `>=`, `>`, `<=`, `<`, `=`, bare literals, partial literals as X-ranges, `||` alternatives OR-ed within a statement; all statements must hold). Rule: (1) parse all `pragma solidity` statements as today (AND across statements, OR inside); (2) if `DEFAULT_SOLC` (0.8.20) satisfies → `DEFAULT_SOLC`; (3) else among the satisfying installed versions take the **lowest satisfying minor** (breaking changes land at minor boundaries) and within that minor the **highest installed patch** that satisfies (features accrue inside a minor; patches are backward compatible — `address[] calldata` on a `public` function needs 0.6.9+); (4) else the legacy nearest-same-minor of the first literal (unknown minor → `0.8.20`). The dense pin set moved 696/864 picks under the old "lowest" rule and broke a `^0.6.0` + `calldata` file (`0x0b1Ff525…` → `Uncertain(compile_failed)`); this rule reproduces the anchor picks for every pre-existing fixture and adds only native exact-pragma picks (`>=0.4.22 <0.6.0` → 0.4.26, `>=0.8.0 <=0.8.10` → 0.8.10, `>=0.8.21` / `>0.8.20` → 0.8.37, `pragma solidity 0.4.24;` → 0.4.24, `pragma solidity 0.8.38;` → 0.8.37).
- `solc_binary(version: str) -> Path` — first existing regular file at `<root>/solc-<v>/solc-<v>` among `solc_artifact_roots()`, else that path under the first root. `solc_artifact_roots()` is an ordered, de-duplicated list: `$DETECTOR_SOLC_ARTIFACTS` when set and non-blank, then `solc_select.constants.ARTIFACTS_DIR`, then `~/.solc-select/artifacts`.
- `oz_trees() -> list[tuple[str, Path]]` — existing trees in declaration order `[("v4", dir), ("v5", dir)]`. v4 dir = `$DETECTOR_OZ_DIR` if set (blank string means none) else `<repo>/vendor/openzeppelin-contracts` else `/app/vendor/openzeppelin-contracts`; v5 dir = `$DETECTOR_OZ_V5_DIR` (same blank semantics) else `<repo>/vendor/openzeppelin-contracts-v5` else `/app/vendor/openzeppelin-contracts-v5`. `oz_remapping(tree: Path | None = None) -> str | None` — `@openzeppelin/contracts/=<dir>/` for the given tree, defaulting to the v4 tree (today's behaviour). `rank_oz_trees(source, trees)` orders by (all `@openzeppelin/contracts/<rel>` specs resolve, then more resolved, then declaration order). `CompileResult.oz_tag` is `"v4"` / `"v5"` when the file imported `@openzeppelin/`, else `None`.
...
**Retry ladder.** Each rung is a fresh `Slither(...)` construction; the ladder stops at the first success and performs at most `MAX_SOLC_ATTEMPTS = 5` constructions per OZ tree. Rewritten sources preserve line numbers exactly (statements are replaced in place, newlines kept) — Slither caches source text at construction, so `source_mapping.lines`/`.content` stay valid.

*Scratch mirror (amended 2026-09-20 after the Docker all-tier run; supersedes "same directory").* The input root is **never written**: the judge mounts it `-v <dir>:/input:ro`, and the same-directory `<stem>.__relaxed__.sol` copy failed there with `[Errno 30] Read-only file system` → `Uncertain(analysis_error)` on 48 Tier 2 files (every ladder-rescued file, e.g. `pragma solidity 0.8.19;`). When a rewrite rung is needed, `compile.py` builds a **scratch mirror** of the input root under the system temp dir (`$DETECTOR_SCRATCH_DIR` if set by the parent, else `tempfile.mkdtemp(prefix="detector-")`): the whole root is mirrored with hard links where the filesystem allows and byte copies otherwise (never symlinks — solc resolves them through `--allow-paths`), the rewritten file **overwrites its own relative path** inside the mirror (so relative imports, `remappings.txt`/`foundry.toml`, `node_modules/` and `lib/<pkg>/` resolve exactly as in the root), `--allow-paths` and project remaps are recomputed against the mirror root, and `CompileResult.source_path` is the mirror path (`canonical_path` stays the original, which is what `results.json` reports). The mirror is built lazily (a file whose first attempt compiles never triggers it), removed by the worker after analysis, and the parent removes the whole scratch root at the end of `analyze_dir` / `run_submission` and after a timeout kill (`cleanup_temp_copies` now also removes that file's mirror). `is_temp_copy` / `TEMP_COPY_SUFFIX` remain for backward compatibility of the directory walk but no such file is created anymore. Rungs: (a) the error says `requires different compiler version` → every `pragma solidity …;` replaced by `pragma solidity >=0.4.0;`, same picked version (`note = "pragma 0.4.25 relaxed; compiled with 0.4.26"`); (b) the file has **no** pragma and the default fails → retry with `0.4.26`, `0.5.17`, `0.6.12`, `0.7.6` in that order (`note = "no pragma; compiled with 0.4.26"`); (c) the error says `Multiple SPDX license identifiers` → every SPDX comment after the first is blanked, combined with (a) when both apply (`note = "duplicate SPDX identifiers removed; compiled with 0.8.20"`); (d) anything else (undeclared identifiers, missing imports, broken syntax at every version) → `CompileError`; (e) OZ tree: ranked trees (`rank_oz_trees`), the whole ladder per tree, `oz <tag> failed: <first error line>` in the attempt log then the next tree; OZ-version: the error contains `requires different compiler version` and names a path under the current OZ tree → newest installed `0.8.x`, then (a) if the user pragma excludes it. Notes record the tree whenever the file imported `@openzeppelin/`: `oz=v5` when the first attempt succeeded on a non-default tree; `oz=v5; compiled with 0.8.37` / `pragma 0.8.20 relaxed; oz=v5; compiled with 0.8.37` when the version/relax rungs ran. Files with no `@openzeppelin/` import keep the existing notes. The ladder never fires on a file whose first attempt compiles, so every pre-existing Tier 1 / Tier 3 fixture still compiles with `note is None`. When the scratch root cannot be created (read-only rootfs without a tmpfs `/tmp`), `scratch_session()` logs once and yields `None`, rewrite rungs are skipped with `no scratch: <error>` in the attempt log, and first-try-compiling files still score; the session never raises.
...
- `auth_atoms(function) -> list[AuthAtom]` — atoms in end nodes (`require`/`assert`/`if`→`revert`) of the function body, its modifiers, and its internal callees (closure). An `if (cond) revert …;` arm counts as a revert whether it is `revert()`, `revert(string)`, or a custom error (`SolidityCustomRevert`), so OZ v5 `_checkOwner` and any 0.8.4+ custom-error gate are end-auth nodes like `require`.
...
- `is_constant_bound(node_condition, ctx) -> bool` — the bounding operand is a literal, `constant`, `immutable`, or an expression over those and `totalSupply()`/supply vars only (no privileged-writable var). `is_constant_bound` resolves condition operands through same-function local definitions (assignment / binary / conversion chains) to state vars, constants, or calls, so a post-write cap check split across nodes (OZ v5 `ERC20Capped._update`) is recognised; a privileged-writable bound reached through a local still disqualifies.
...
| `LEAK_ARBITRARY_TRANSFERFROM` | C | HIGH | a `bal` debit at a non-sender key reachable under privilege (function or branch mode) with no read of an allowance var dominating it on that path; debits whose key parameter is bound to the zero address on every internal call path from the entry function are not holder debits and are skipped (OZ v5 unified `_update` reached from `_mint`) | `issuer_token`→MED; `managed_role`→MED |
...
`python:3.11-slim`; `git`; `dpkg --add-architecture amd64 && apt-get install libc6:amd64` (portable to arm64 hosts, see `Dockerfile.bench`); `pip install -r detector/requirements.txt`; `ENV HOME=/opt/detector-home DETECTOR_SOLC_ARTIFACTS=/opt/detector-home/.solc-select/artifacts` and `mkdir -p /opt/detector-home`; `COPY detector/solc_versions.txt` then `solc-select install` each line of that file (no hand-written versions) so the binaries land under that HOME; `COPY detector /app/detector`, `COPY vendor/openzeppelin-contracts /app/vendor/openzeppelin-contracts`, `COPY vendor/openzeppelin-contracts-v5 /app/vendor/openzeppelin-contracts-v5`; `chmod -R a+rX /opt/detector-home /app` (any uid can read the tree and execute solc); no `USER` directive (bench mode must still write a root-owned `/output` when run as root). The image must run under any uid and under `--read-only`. `ENV DETECTOR_OZ_DIR=/app/vendor/openzeppelin-contracts DETECTOR_OZ_V5_DIR=/app/vendor/openzeppelin-contracts-v5 PYTHONPATH=/app`; `ENTRYPOINT ["python","-m","detector.docker_entry"]` — a thin launcher: `DETECTOR_MODE=submission|bench` overrides the `/output` auto-detect; if unset or unknown, a mounted writable `/output` → bench mode (`/input` → `/output/results.json` + `summary.md`); otherwise → submission mode (JSON array on stdout). Image `trust404/detector:latest`. `run.sh <dir>` at the repo root runs the image in submission mode (`--network none`, `:ro` input) and falls back to the local venv when Docker is unavailable.
...
## Acceptance (DT-1 .. DT-20) — status column is updated per phase; rows are never removed

| ID | Criterion | Status |
|---|---|---|
| DT-1 | `detector` is a registered bench tool with a Docker image; `bench run detector` completes under `--network none` on all tiers; output is schema-valid; determinism passes | met 2026-09-20 — Docker all-tier `--network none --repeat 2` (image from `6aee5fb`, re-confirmed on `e0a089bddd3a` after the hardening waves): 0.9714, schema-valid, determinism pass, 863 cases (`reports/detector/`) |
| DT-2 | `_harness/compile_fail` → Uncertain(`compile_failed`); `STRUCT_EXTERNAL_GATE/mal` → Uncertain(`external_dependency`); `oz_import` and `multi_file` are not Malicious | met — Tier 1: `compile_fail` → `compile_failed`, `STRUCT_EXTERNAL_GATE/mal` → `external_dependency`, all 67 exact incl. `oz_import`/`multi_file` |
| DT-3 | Tier 1 rule recall = 1.0 (every `mal` twin fires its `expected_rule_id`); floor 0.9 | met — Tier 1 rule recall 1.0 |
| DT-4 | Tier 1 benign twins: HIGH-FP rate = 0; floor ≤ 0.05 | met — Tier 1 HIGH-FP 0.0 |
| DT-5 | Tier 3: HIGH-FP rate = 0 on the five bounded fixtures (`oz_erc20_pausable_ownable`, `oz_erc20capped_accesscontrol`, `oz_erc20permit`, `reflection_token`, `erc20_foreign_rescue`), each Benign; the three governance-only fixtures (`usdc_fiattoken`, `bancor_smarttoken`, `lido_ldo_minime`) resolve to Malicious or Benign, never `Uncertain(med_findings)` — `lido_ldo_minime` may resolve `Uncertain(external_dependency)` because its transfer path calls the settable `controller` (the one analytic abstain; its label accepts Uncertain for that reason) (amended 2026-09-20 with the label change in `baybench.md`) | met — five bounded Benign; `usdc`/`bancor` Malicious; `lido` Uncertain(`external_dependency`); Tier 3 HIGH-FP 0.0 |
| DT-6 | Tier 2 family recall ≥ 0.6 on the compiling sources; per-family numbers recorded; every miss bucketed | met — Tier 2 family recall 0.9146 (A 0.978 / B 0.867 / C 1.0 / F 0.910); every miss bucketed in `misses.md` Runs 3–6 |
| DT-7 | Evidence hit rate ≥ 0.9 on Tier 1 `mal` (`expected_functions`); every finding has contract, function, lines, reasoning | met — evidence hit rate 1.0 on Tier 1 mal; every finding carries contract/function/lines/reasoning |
| DT-8 | Name-agnostic: identifier-renaming test on `PRIV_ROLE`, `OWN_HIDDEN_ROLE`, `OWN_TX_ORIGIN`, `EXIT_ADDR_GATE` mal fixtures yields identical rule IDs; no rule module contains identifier-name matching | met — `test_rename_invariance.py` + static name-list guard pass |
| DT-9 | Beats baselines on the same tier set as the baseline reports (Tier 1 + Tier 3): weighted_score above `baseline_slither` (0.5789) and `baseline_keyword` (0.4222); Tier 3 HIGH-FP rate below the keyword baseline's 0.75 | met — Tier 1+3 weighted 0.9844 vs 0.5789 / 0.4222; Tier 3 HIGH-FP 0.0 < 0.75 |
| DT-10 | Runtime: Tier 1+3 in Docker under 5 minutes; per-file timeout yields Uncertain(`timeout`), never a crash | met — Tier 1+3 in Docker 35 s (all tiers 6.4 min per repeat on an arm64 host under qemu); timeout → Uncertain(`timeout`) covered by `test_engine.py` |
| DT-11 | Judge packaging: one `docker run --rm --network none -v in:/input:ro -v out:/output trust404/detector` produces `results.json` and a human-readable `summary.md`; README explains verdict derivation | met — `docker run … -v out:/output` writes `results.json` + `summary.md`; README "Verdict derivation" section is the decisive ladder |
| DT-12 | Iterate loop evidenced: `docs/bench/misses.md` has one triaged row per first-run gap with bucket, fix, status; `reports/detector/` committed | met — `misses.md` Runs 1–6 + probes, one row per gap; `reports/detector/` committed with the Docker all-tier report |
| DT-13 | **Submission contract** (added 2026-09-20 from `challenge_public/`): `./run.sh <dir>` and `docker run --rm --network none -v <dir>:/input:ro trust404/detector` print one `schema.json`-valid JSON array to stdout (logs on stderr, exit 0); `MALICIOUS` objects carry evidence with in-range lines; only top-level `*.sol` are emitted with basename `file`; global budget yields the array before the organizers' 10-min kill; **read-only input** (amended 2026-09-20): a file that needs a ladder rewrite (exact pragma not installed, duplicate SPDX, BOM) gets the same verdict under `:ro` as from a writable directory — the Docker all-tier run must show zero `analysis_error` and per-file verdicts identical to the local run | met — `run.sh` and `docker … /input:ro` print one schema-valid array (Tier 0, 3-file `:ro` repro); read-only clause: Docker all-tier has 0 `analysis_error` and Tier 2 verdicts identical to the local run (`6aee5fb`) |
| DT-14 | **Tier 0 = 5/5** on the public samples with the organizers' labels (`P1`, `P4` BENIGN; `P2`, `P3`, `P5` MALICIOUS), `reasons` non-empty and consistent with the verdict for every file; Tier 1/3 verdicts move only toward `preferred_verdict` under decisive mode | met — Tier 0 5/5 with non-empty reasons; Tier 1/3 flips under decisive mode all toward `preferred_verdict` (Run 5/6) |
| DT-15 | **Decisive** (2026-09-20): across Tiers 0–3, `Uncertain` appears only with reason ∈ {`compile_failed`, `timeout`, `analysis_error`, `external_dependency`}; Tier 1 rule recall stays 1.0; the five bounded Tier 3 fixtures are Benign with zero HIGH | met — Uncertain reasons across Tiers 0–3: `compile_failed`, `external_dependency` only; Tier 1 rule recall 1.0; five bounded Tier 3 fixtures Benign, zero HIGH |
| DT-16 | **Bench doctrine sync** (2026-09-20): the label amendments (Tier 1 decisive-MED `mal` twins, `FEE_ADDR_MUTABLE/mal`, `PRIV_ROLE/ben` re-shape, Tier 3 governance trio) and `scoring.py` Tier 0 weight 1.0 + `tier0 exact k/n` gate line land in the same commit as the policy; `bench validate` clean on all tiers; `docs/bench/misses.md` carries one row per moved verdict citing the organizers' rule | met — labels + `scoring.py` landed with the policy in `ffd3b8d`; `bench validate` clean on all tiers (2026-09-20); `misses.md` Run 5/6 rows cite the organizers' rules |
| DT-17 | **Any-uid run** (2026-09-20): `docker run --rm --network none -v <dir>:/input:ro` as root, as `--user 1000:1000`, and as `--user 65534:65534 --read-only --tmpfs /tmp --cap-drop ALL --security-opt no-new-privileges --memory 4g --cpus 2 --pids-limit 512` all print one schema-valid array with Tier 0 = 5/5, byte-identical stdout across the three; `--read-only` without tmpfs still prints a valid array (ladder-dependent files degrade to `UNCERTAIN`, never an empty array); `DETECTOR_MODE=submission` forces the stdout array even with `/output` mounted | met 2026-09-20 — `tests/detector/test_docker_hardening.py` (7 rows: root, uid 1000, uid 65534 hardened, `--read-only` without tmpfs, `DETECTOR_MODE` both ways) green with `DETECTOR_DOCKER_TESTS=1` against `trust404/detector:latest` (`e0a089bddd3a`); stdout `cmp`-identical across the three uids; Tier 0 5/5 in every row |
| DT-18 | **Published amd64 image** (2026-09-20): a native-amd64 image is published to `ghcr.io/sdh2222/trust404-detector` on `workflow_dispatch` and `push` of `submission-*` tags; the image digest and the `trust404-detector-amd64.tar.gz` release-asset sha256 are recorded in the root README (`<DIGEST-TBD>` until CI fills it); CI hardened smoke (`scripts/judge_smoke.sh` plus uid 1000 and uid 65534 / read-only / cap-drop / 2 vCPU / 4 GB) is green on that digest and the three stdout arrays are byte-identical | OPEN — workflow `.github/workflows/detector-image.yml` checked in (Wave C, `tests/detector/test_workflow.py`); digest pending the `submission-rc1` run |
| DT-19 | **Cold-clone path** (2026-09-20): the root README "judges start here" section documents `./run.sh ./cases > out.json`; when no Docker image is present locally and the chosen interpreter cannot `import slither`, `run.sh` exits 2 with stderr guidance (`docker load` / `docker build --platform linux/amd64` / `scripts/setup_local.sh`) and empty stdout; `scripts/setup_local.sh` makes the `.venv` path work | met — root README leads with the submission section; `run.sh` backend selection + `exit 2` covered by `tests/detector/test_run_sh.py` (Wave C); `scripts/setup_local.sh` reads `solc_versions.txt` and is idempotent (Wave B) |
| DT-20 | **solc + OpenZeppelin coverage** (2026-09-20): `INSTALLED_SOLC` is loaded from `detector/solc_versions.txt` and the Dockerfile installs exactly that list (no hand-written versions); exact pragmas `0.8.0..0.8.37`, `0.4.24/25`, `0.5.16`, `0.6.6` compile natively (`note is None`); OZ v4 and v5 trees are vendored and chosen per file (`rank_oz_trees` + per-tree ladder + OZ-version rung); the four `_harness/oz_v*` fixtures compile against the right tree and hold their labelled verdicts (Malicious / Benign / Benign / Benign); every pre-existing Tier 1/3 fixture still compiles with `note is None`; `bench validate --tier 1` clean | met 2026-09-20 — `test_installed_solc_matches_versions_file` + `test_dockerfile_installs_from_solc_versions_file`; exact-pin table in `test_compile.py` (`note is None` for 0.8.19, 0.4.24, 0.5.16, …); `rank_oz_trees` / per-tree / version-rung tests; `test_analyze_dir_oz_v_harness` quartet Malicious / Benign / Benign / Benign; `bench validate --tier 1` 71/71 and all tiers 863/863; Docker all-tier run Tier 1 71/71 exact with every previous verdict unchanged |
...
- 6 Judge-run hardening (plan `judge-run_hardening`, 2026-09-20): Wave A any-uid / read-only runtime (`DETECTOR_SOLC_ARTIFACTS`, scratch degrade, `DETECTOR_MODE`); Wave B solc SSOT + OZ v4/v5 tree selection + the three analysis fixes the v5 fixtures exposed (`is_revert_ir` custom errors, `constant_cap` through locals, zero-address dead-debit skip); Wave C `run.sh` backends, `scripts/judge_smoke.sh`, judges-first README, amd64 CI image. → DT-17, DT-18, DT-19, DT-20.

## Known risks (recorded, not scope changes)

- `is_revert_ir` treated `"revert "` as an exact `REVERT_NAMES` member, so Slither `SolidityCustomRevert` (`revert OwnableUnauthorizedAccount(address)`) was not a revert arm; OZ v5 `_checkOwner` and 0.8.4+ `if (…) revert CustomError()` gates dropped `auth_atoms` (found 2026-09-20 via `_harness/oz_v5_unbounded_mint`). Fixed by matching `SolidityCustomRevert` and a `"revert "` prefix.
- `is_constant_bound` / `_reads_supply` looked only inside the condition node, so OZ v5 `ERC20Capped._update` (`uint256 maxSupply = cap(); uint256 supply = totalSupply(); if (supply > maxSupply) revert`) was not a `constant_cap` (found 2026-09-20 via `_harness/oz_v5_capped_mint`). Fixed by resolving condition operands through same-function local definitions.
- `LEAK_ARBITRARY_TRANSFERFROM` counted the dead `from != 0` debit in OZ v5 unified `_update` when `_mint` bound `from` to `address(0)` (found 2026-09-20 via `_harness/oz_v5_capped_mint`). Fixed by skipping debits whose key parameter is zero on every internal-call path from the entry.
