...
- `compile_file(path: Path) -> Slither` — constructs `Slither(str(path), solc=<bin>, solc_remaps=[remap], solc_args="--allow-paths <parent>,<oz>")`; raises `CompileError(message)` on any failure (message includes solc version and first 500 chars of stderr).
