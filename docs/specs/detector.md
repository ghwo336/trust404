...
**Retry ladder.** Each rung is a fresh `Slither(...)` construction; the ladder stops at the first success and performs at most `MAX_SOLC_ATTEMPTS = 5` constructions per file. Rewritten sources preserve line numbers exactly (statements are replaced in place, newlines kept) — Slither caches source text at construction, so `source_mapping.lines`/`.content` stay valid.
...
## Acceptance (DT-1 .. DT-16) — status column is updated per phase; rows are never removed
